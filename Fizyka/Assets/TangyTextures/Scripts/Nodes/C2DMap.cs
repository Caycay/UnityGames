 ...
 * */
		...
		public class C2DRGBMap
		{
				...
				public void Combine (C2DRGBMap a, C2DRGBMap b, C2DMap combiner, float blendval, int type)
				{
						for (int i=0; i<C2DMap.sizeX; i++)
								for (int j=0; j<C2DMap.sizeY; j++) {
										float t = blendval;
										if (combiner != null)
												t = Mathf.Clamp (combiner.map [i, j] * t, 0, 1);
					
										if (type == RGBCombiner.TYPE_BLEND)
												colors [i, j] = a.colors [i, j] * t + b.colors [i, j] * (1 - t);
										if (type == RGBCombiner.TYPE_SUB)
												colors [i, j] = a.colors [i, j] * t - b.colors [i, j] * (1 - t);
										if (type == RGBCombiner.TYPE_MUL)
												colors [i, j] = a.colors [i, j] * b.colors [i, j];
										if (type == RGBCombiner.TYPE_OVERRIDE) {
												float v = 2 * a.colors [i, j].r;
												v = Mathf.Max (v, a.colors [i, j].g);
												v = Mathf.Max (v, a.colors [i, j].b);
												v = Mathf.Pow (v, blendval * 5f);
												colors [i, j] = v * a.colors [i, j] + (1 - v) * b.colors [i, j];
										}
										if (type == RGBCombiner.TYPE_MIN)
												colors [i, j] = Min (a.colors [i, j],b.colors [i, j]);
										if (type == RGBCombiner.TYPE_MAX)
												colors [i, j] = Max (a.colors [i, j],b.colors [i, j]);
		...
		public class C2DMap
		{
				...
				public void Combine (C2DMap a, C2DMap b, C2DMap combiner, float blendval, int type)
				{
						for (int i=0; i<sizeX; i++)
								for (int j=0; j<sizeY; j++) {
										float t = blendval;
										if (combiner != null)
												t = Mathf.Clamp (combiner.map [i, j] * t, 0, 1);
										if (type == HeightCombiner.TYPE_BLEND)
												map [i, j] = a [i, j] * t + b [i, j] * (1 - t);
										if (type == HeightCombiner.TYPE_SUB)
												map [i, j] = a [i, j] * t - b [i, j] * (1 - t);
										if (type == HeightCombiner.TYPE_MUL)
												map [i, j] = a [i, j] * b [i, j];
										if (type == HeightCombiner.TYPE_MIN)
												map [i, j] = Mathf.Min (a [i, j] , b [i, j]);
										if (type == HeightCombiner.TYPE_MAX)
												map [i, j] = Mathf.Max(a [i, j] , b [i, j]);
				...
				public void calculatePerlin (float amplitude, float scale, float octaves, float kscale, float seed, float perlinSkew, float perlinSkewScale)
				{
						...
						for (int i=0; i<sizeX; i++) {
								for (int j=0; j<sizeY; j++) {
										...
										float p = Util.perlinNoise2dSeamless (1, 1, 1.0f, x * perlinSkewScale, y * perlinSkewScale, 0, 0, 0, 0);
										float t = perlinSkew;
										float xx = x + t * p;
										float yy = y + t * p;
										CNodeManager.Progress (1.0f / (sizeX * sizeY));
					
										map [i, j] = amplitude * (0.5f + Util.perlinNoise2dSeamless (octaves, scale, kscale, xx, yy, 0, 0, 0, 0));// / Mathf.Pow(k, kscale);///(float)kk;
								}
						}
				}
				...
				public void calculateMultiridged (float seed, float heightScale, float frequency, float lacunarity, float gain, float offset, float ioffset)
				{
						...
						for (int i=0; i<nx; i++) {
								for (int j=0; j<ny; j++) {
										...
										float v = Util.getRidgedMf (p, frequency, (int)octaves, lacunarity, 0, offset, gain, ioffset);
										map [i, j] = v * heightScale;
										CNodeManager.Progress (1.0f / (nx * ny));
						...
						for (int i=S; i<D; i++) {
								for (int x = 0; x < sizeX; x++) {
										for (int y = 0; y < sizeY; y++) {
												...
												CNodeManager.Progress (1.0f / (D * sizeX * sizeY));
